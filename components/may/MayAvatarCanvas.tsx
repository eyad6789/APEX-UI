...
import MayAtmosphere from "./MayAtmosphere";
import MayContourField from "./MayContourField";
import MayEnergyCore from "./MayEnergyCore";
import MayParticleField from "./MayParticleField";
...
function AvatarRig({ state, quality, reducedMotion, audioFrameRef }: Omit<Props, "onContextFailure">) {
  const groupRef = useRef<THREE.Group>(null);
  useFrame(({ pointer, camera, clock }, delta) => {
    if (!groupRef.current) return;
    const amount = reducedMotion ? 0 : 1;
    groupRef.current.rotation.y = THREE.MathUtils.damp(groupRef.current.rotation.y, pointer.x * .105 * amount, 3.2, delta);
  ...
  return (
    <group ref={groupRef}>
      <MayAtmosphere reducedMotion={reducedMotion} />
      <MayContourField state={state} quality={quality} reducedMotion={reducedMotion} audioFrameRef={audioFrameRef} />
      <MayParticleField state={state} quality={quality} reducedMotion={reducedMotion} audioFrameRef={audioFrameRef} />
      <MayEnergyCore state={state} reducedMotion={reducedMotion} audioFrameRef={audioFrameRef} />
    </group>
  );
}
      ...
      >
        ...
        {config.bloom && !props.reducedMotion ? (
          <EffectComposer multisampling={props.quality === "ultra" ? 4 : 0}>
            <Bloom intensity={.82} luminanceThreshold={.18} luminanceSmoothing={.72} mipmapBlur />
          </EffectComposer>
        ) : null}
      </Canvas>
    </MaySceneBoundary>
